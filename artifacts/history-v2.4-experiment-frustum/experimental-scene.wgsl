...
fn transformEntity(sourceEntity:Entity)->Entity{var e=sourceEntity;var a=DS(e.anchor.xy,e.anchor.zw);var m=mat2x2<f32>(vec2<f32>(1.,0.),vec2<f32>(0.,1.));var n=e.data.z;
 for(var depth=0u;depth<128u&&n!=0xffffffffu;depth++){
...
fn calculateTextBasis(e:Entity)->mat2x2<f32>{var height=e.p.x;var width=e.p.y;var angle=e.p.z;
 ...
 let c=cos(angle);let s=sin(angle);return entityMatrix(e)*mat2x2<f32>(vec2<f32>(c,s)*height*width,vec2<f32>(c*tan(e.p.w)-s,s*tan(e.p.w)+c)*height);
}
...
fn cullClip()->vec4<f32>{let center=frame.camera.xy+frame.camera.zw;let h=frame.viewport.xy*(.5+frame.grid.x)/frame.viewport.z;let co=abs(frame.pointer.x);let si=abs(frame.pointer.y);let half=vec2<f32>(co*h.x+si*h.y,si*h.x+co*h.y)+max(4.*frame.viewport.w,(3.+.5*frame.settings.z)*frame.viewport.w+1.)/frame.viewport.z;return vec4<f32>(center-half,center+half);}
var<workgroup> clusterClip:vec4<f32>;
fn boxOnScreen(b:vec4<f32>)->bool{return all(b.zw>=clusterClip.xy)&&all(b.xy<=clusterClip.zw);}
...
fn cullScan(@builtin(global_invocation_id) gid:vec3<u32>,@builtin(local_invocation_index) lane:u32,@builtin(workgroup_id) group:vec3<u32>){
 if(lane==0u){clusterClip=cullClip();clusterActive=u32(boxOnScreen(bounds[header.counts.x+group.x]));}
